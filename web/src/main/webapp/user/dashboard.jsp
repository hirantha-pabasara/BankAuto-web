    ...
    function performLogout() {
        if (confirm('Are you sure you want to logout?')) {
            try {
                ...
                // Send GET request to the logout servlet
                fetch('/user/logout', {  // Adjust the path if your context root differs (e.g., '/app/user/logout')
                    method: 'GET',
                    credentials: 'include'  // Ensures cookies/session data are sent with the request
                })
                    .then(response => {
                        if (response.redirected) {
                            // Follow the servlet's redirect automatically
                            window.location.href = response.url;
                        } else if (response.ok) {
                            // Fallback: Redirect manually if no server redirect occurs
                            window.location.href = '../user/login.jsp';  // Or your desired post-logout page
                        } else {
